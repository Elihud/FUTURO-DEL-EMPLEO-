...
amigeshion[H]={"\t Lunes","\t Martes","\t Miercoles","\t Jueves","\t Viernes","\t Sabado","\t Domingo"};
struct agend
{
char aa[50];
char as[50];
...
switch(opc){
    ...
    case 3:
        ...
        printf("%s\n",output);}
                    ...
                    else if (opccion==2){printf("Los cambios se veran en tu agenda social se veran reflejados el proximo mes.\n\n\t\t\t AYUNTAMIENTO");
                    time_t tiempo = time(0);
        struct tm *tlocal = localtime(&tiempo);
        char output[128];
        strftime(output,128,"%d/%m/%y %H:%M:%S",tlocal);
        printf("%s\n",output);}
                    else if(opccion>2&&opccion<1) {printf("La opcion no se encuentra");}
/*                getch();*/
					int vcv;
					for(vcv=0;vcv<250000;++vcv){}
                break;
    case 4:
           break;
         }
...
else {printf("La opcion elegida NO es CORRECTA");}
}
